...
public class RotatableBondsCountDescriptor implements IMolecularDescriptor {
    ...
    public void calculate1( IAtomContainer ac, Writer writer, List<String> atomFromPDB ) throws Exception
    {
        int degree0;
        int degree1;
        IRingSet ringSet;
        ...
        int[][] topologicalDistances = TopologicalMatrix.getMatrix( ac );
                        ...
                        (BondManipulator.isLowerOrder(ac.getMaximumBondOrder(atom1), IBond.Order.TRIPLE))) 
                {
                    if (!bond.getFlag(CDKConstants.ISINRING)) 
                    {
                        ...
                        if ((degree0 == 1) || (degree1 == 1)) 
                        {
                            if (includeTerminals) 
                            {
                                writer.write( atomFromPDB.get( ac.getAtomNumber( atom0 ) ) + "-" + atomFromPDB.get( ac.getAtomNumber( atom1 ) ) + "\r\n" );
                                calculate1Aux( topologicalDistances, ac, atom0, atom1, writer, atomFromPDB );
                            }
                        } 
                        else 
                        {
    ...
    private void calculate1Aux( int[][] topologicalDistances, IAtomContainer ac, IAtom atom0, IAtom atom1, Writer writer, List<String> atomFromPDB ) throws Exception
    {
        List<IAtom> connectedAtoms2Atom0 = ac.getConnectedAtomsList( atom0 );
        connectedAtoms2Atom0.remove( atom0 );
        connectedAtoms2Atom0.remove( atom1 );                            
        List<IAtom> connectedAtoms2Atom1 = ac.getConnectedAtomsList( atom1 );
        connectedAtoms2Atom1.remove( atom0 );
        connectedAtoms2Atom1.remove( atom1 );
        
        int posAtom0 = ac.getAtomNumber( atom0 );
        int posAtom1 = ac.getAtomNumber( atom1 );

        for ( IAtom atomCon20 : connectedAtoms2Atom0 )
        {
            if ( !atomCon20.equals( atom1 ) )
            {
                int posAtomCon20 = ac.getAtomNumber( atomCon20 );
                if ( topologicalDistances[posAtomCon20][posAtom0] == 1 )
                {
                    for ( IAtom atomCon21 : connectedAtoms2Atom1 )
                    {
                        if ( !atomCon21.equals( atom1 ) )
                        {
                            int posAtomCon21 = ac.getAtomNumber( atomCon21 );
                            if ( topologicalDistances[posAtomCon21][posAtom1] == 1 )
                            {
                                writer.write( atomFromPDB.get( posAtomCon20 )              + "-" +
                                              atomFromPDB.get( ac.getAtomNumber( atom0 ) ) + "-" + atomFromPDB.get( ac.getAtomNumber( atom1 ) ) + "-" +
                                              atomFromPDB.get( posAtomCon21 )              + "\r\n" );
                            }
                        }
                    }
                }                                    
