...
if [ -e scan_sander_basal/out_3_$rootname  ]; then rm scan_sander_basal/out_3_$rootname/*; else mkdir scan_sander_basal/out_3_$rootname; fi
for N in $number;
do
...
sanderexe=`which sander` 
 ...
 $sanderexe -O -i input.in -o scan_sander_basal/out_3_$rootname/${rootname}_$N.out -p scan_sander_basal/${rootname}.prmtop -c scan_sander_basal/${rootname}.inpcrd -r scan_sander_basal/out_3_$rootname/${rootname}_$N.inpcrd
done
...
for file in scan_sander_basal/out_3_$rootname/*.out;
do
...
awk '{printf "%.7f\n",$3}' sander_energies.txt > scan_sander_basal/out_3_$rootname/sander_3_$rootname.txt
...
do
	if [ $N -lt 10 ]; then N=00$N; fi
	if [ $N -lt 100 -a $N -gt 9 ]; then N=0$N; fi
	ambpdb -p scan_sander_basal/${rootname}.prmtop -c scan_sander_basal/out_3_$rootname/${rootname}_$N.inpcrd > scan_sander_basal/min_pdbs/min_pdb_${rootname}_$N.pdb 2> /dev/null
	...
	cat scan_sander_basal/min_pdbs/min_pdb_${rootname}_$N.sd >> scan_sander_basal/out_3_$rootname/min_pdb_all_3.sd
done
...
rm $rootname.tleap
rm leap.log
rm scan_sander_basal/out_3_$rootname/${rootname}*.inpcrd
rm scan_sander_basal/out_3_$rootname/${rootname}*.out
