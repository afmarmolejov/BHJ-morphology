...
tleapfile="
loadAmberParams frcmod_basal_param/$rootname.frcmod\n
LIG = loadmol2 mol2_basal_0/$rootname.mol2\n
check LIG\n
charge LIG\n
saveamberparm LIG scan_sander_basal_param/$rootname.prmtop scan_sander_basal_param/$rootname.inpcrd\n
saveOff LIG scan_sander_basal_param/$rootname.lib\n
quit\n";
...
if [ -e scan_sander_basal_param/out_3_$rootname  ]; then rm scan_sander_basal_param/out_3_$rootname/*; else mkdir scan_sander_basal_param/out_3_$rootname; fi
for N in $number;
do
...
sanderexe=`which sander` 
 #$AMBERHOME/exe/sander -O -i input.in -o out_$rootname/${rootname}_$N.out -p ${rootname}.prmtop -c ${rootname}.inpcrd -r out_$rootname/${rootname}_$N.rst
 $sanderexe -O -i input.in -o scan_sander_basal_param/out_3_$rootname/${rootname}_$N.out -p scan_sander_basal_param/${rootname}.prmtop -c scan_sander_basal_param/${rootname}.inpcrd -r scan_sander_basal_param/out_3_$rootname/${rootname}_$N.inpcrd
done
...
for file in scan_sander_basal_param/out_3_$rootname/*.out;
do
...
awk '{printf "%.7f\n",$3}' sander_energies.txt > scan_sander_basal_param/out_3_$rootname/sander_3_$rootname.txt

#########OPTIONAL################
#create a sd file for visualization of all sander process in min_pdbs
if [ -e scan_sander_basal_param/min_pdbs ]; then rm scan_sander_basal_param/min_pdbs/*; else mkdir scan_sander_basal_param/min_pdbs; fi
for N in $number;
do
	if [ $N -lt 10 ]; then N=00$N; fi
	if [ $N -lt 100 -a $N -gt 9 ]; then N=0$N; fi
	ambpdb -p scan_sander_basal_param/${rootname}.prmtop -c scan_sander_basal_param/out_3_$rootname/${rootname}_$N.inpcrd > scan_sander_basal_param/min_pdbs/min_pdb_${rootname}_$N.pdb 2> /dev/null
	babel -ipdb scan_sander_basal_param/min_pdbs/min_pdb_${rootname}_$N.pdb -osd scan_sander_basal_param/min_pdbs/min_pdb_${rootname}_$N.sd 2> /dev/null
	cat scan_sander_basal_param/min_pdbs/min_pdb_${rootname}_$N.sd >> scan_sander_basal_param/out_3_$rootname/min_pdb_all_3.sd
done
...
rm $rootname.tleap
rm leap.log
rm scan_sander_basal_param/out_3_$rootname/${rootname}*.inpcrd
rm scan_sander_basal_param/out_3_$rootname/${rootname}*.out
rm scan_sander_basal_param/$rootname.lib
rm scan_sander_basal_param/$rootname.inpcrd
rm scan_sander_basal_param/$rootname.prmtop
rm -r scan_sander_basal_param/min_pdbs/
rm -r dih_tmp/
############################
